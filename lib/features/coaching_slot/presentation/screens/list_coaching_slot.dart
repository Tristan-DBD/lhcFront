...
import '../widgets/coaching_slot_list_tile.dart';
...
class ListCoachingSlotPage extends StatefulWidget {
  ...
  @override
  State<ListCoachingSlotPage> createState() => _ListCoachingSlotPageState();
}

class _ListCoachingSlotPageState extends State<ListCoachingSlotPage>
    with SingleTickerProviderStateMixin {
  late final CoachingSlotController _controller;
  late AnimationController _pulseController;

  @override
  void initState() {
    super.initState();
    _controller = CoachingSlotController();
  ...
  Future<void> _handleBook(String slotId) async {
    ...
    _showSnackBar(
      success
          ? 'Réservation du créneau réussie'
  ...
  Future<void> _handleCancel(String slotId) async {
    ...
    _showSnackBar(
      success
          ? 'Annulation de la réservation réussie'
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        final dailySlots = _controller.getSlotsForDay(_controller.selectedDay);
  ...
  Widget _buildCalendar() {
    return Container(
      ...
      child: TableCalendar<CoachingSlot>(
        locale: 'fr_FR',
        ...
        eventLoader: (day) => _controller.getSlotsForDay(day),
        ...
        if (_controller.userRole == 'ADMIN' || _controller.userRole == 'COACH')
          IconButton(
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => CreateCoachingSlotScreen(
                  onSlotCreated: () => _controller.loadSlots(),
                ),
              ),
            ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Il n\'y a pas encore de créneaux pour cette date. Revenez plus tard ou contactez votre coach.',
  ...
  Widget _buildSlotList(List<CoachingSlot> dailySlots) {
    return ListView.builder(
      padding: const EdgeInsets.symmetric(vertical: 8),
      itemCount: dailySlots.length,
      itemBuilder: (context, index) {
        final slot = dailySlots[index];
        return CoachingSlotListTile(
          slot: slot,
          controller: _controller,
          onEdit: () => Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => EditCoachingSlotScreen(
                slot: slot,
                onSlotUpdated: () => _controller.loadSlots(),
              ),
            ),
          ),
          onDelete: () => _handleDelete(slot.id),
          onBook: () => _handleBook(slot.id),
          onCancel: () => _handleCancel(slot.id),
        );
      },
    );
  }
